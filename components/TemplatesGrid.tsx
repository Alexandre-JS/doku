...
const getCachedTemplates = unstable_cache(
  async (limit?: number) => {
    ...
    const { data: templData, error } = await query;
...
export default async function TemplatesGrid({ limit }: TemplatesGridProps) {
  try {
    const allTemplates = await getCachedTemplates(limit);
    
    const templates = limit ? allTemplates.slice(0, limit) : allTemplates;
    const hasMore = limit ? allTemplates.length > limit : false;
    ...
    return (
      <div className="col-span-full py-20 text-center text-red-500">
        Erro ao carregar os modelos. Por favor, tente novamente mais tarde.
      </div>
    );
  }
}
